...
from src.future_climate_pipeline import (
    load_future_month_for_experiment,
...
def test_load_future_month_for_experiment(monkeypatch):
    config = make_config()
    ...
    def fake_load_chelsa_future_monthly_subset(
        variable,
        month,
        gcm,
        scenario,
    ...
    monkeypatch.setattr(
        "src.future_climate_pipeline.load_chelsa_future_monthly_subset",
        fake_load_chelsa_future_monthly_subset,
    )

    result = load_future_month_for_experiment(
        experiment=experiment,
        municipalities_gdf=municipalities,
        month=7,
        chunks={
    ...
    assert captured["gcm"] == "MRI-ESM2-0"
    assert captured["scenario"] == "ssp370"
    assert captured["period"] == "2041-2070"
    assert captured["chunks"] == {
        "lat": 250,
        "lon": 250,
    }
